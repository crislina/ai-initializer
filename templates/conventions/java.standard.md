...
- **Guardrails** are hard constraints for formal Java projects, services, libraries, CLI tools, and batch jobs unless an explicit repository requirement justifies an exception.
...
- Do not use broad exception handling outside a deliberate system boundary.
...
- Do not weaken tests, type checks, linting, formatting, static analysis, or security checks merely to make the build pass.
...
- Concurrency, file handling, command execution, serialization, and integration behavior must be verified with realistic execution when they are in scope.
...
- Make retry behavior explicit for state-changing or externally visible operations.
- Use shared mechanisms such as database constraints, atomic statements, durable queues, leases, or distributed locks when correctness must hold across instances.
- Keep concurrent code bounded and avoid unbounded thread creation.
- Use `Instant` for absolute instants.
...
- Use real overlapping threads, processes, or transactions when concurrency guarantees are in scope.
...
- Review maintenance, security, license, platform, and transitive impact before adding a dependency.
- Keep package metadata, entry points, manifests, and supported Java versions accurate.
